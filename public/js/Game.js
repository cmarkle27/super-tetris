jQuery(document).ready(function($) {
    ...
    var socketMsg = io.connect("http://localhost:3700");

  // socketMsg.on("message", function(message) {
  ...
  // });
    ...
    socketMsg.emit("gameReady");

    socketMsg.on("startGame", function() {
        $('#start').trigger("click");
    });

    socketMsg.on("movePiece", function(data) {
        console.log(data);
        tetris.movePiece(data.direction);
    });
